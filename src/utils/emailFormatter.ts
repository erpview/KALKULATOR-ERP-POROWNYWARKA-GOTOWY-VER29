...
const CONSULTANT_NAMES = {
  zelinski: 'Jarosław Żeliński - IT Consulting',
  pieklik: 'Janusz Pieklik - Business Global Consulting Polska Sp. z o.o.'
};
...
export const formatEmailTemplate = (formData: FormData) => {
  ...
  // Format selected consultants
  const selectedConsultants = formData.selectedConsultants
    .map(id => CONSULTANT_NAMES[id])
    .join('\n');
  ...
  const templateParams = {
    ...
    selected_consultants: selectedConsultants || 'Brak wybranych konsultantów',
